 ...
 * optional keys {@code :on-connect}, {@code :on-message}, {@code :on-close} and
 * {@code :on-error}.
 *
 * <p>Implemented in Java because Clojure's proxy is slow and does not allow to
 * call super methods.
 */
public class WebSocketChannelListener
    extends AbstractReceiveListener implements WebSocketConnectionCallback
{
  ...
  @Override public void onConnect (WebSocketHttpExchange exchange,
  ...
  @Override protected void onFullTextMessage (WebSocketChannel channel,
    ...
    if (this.onMessage != null)
      onMessage.invoke( RT.map( k_callback, k_onMessage,
                                k_channel, channel,
                                k_text, text.getData() ) );
    else super.onFullTextMessage( channel, text );
  }

  @Override protected void onFullBinaryMessage (WebSocketChannel channel,
    ...
    else super.onFullBinaryMessage( channel, binary );
  }
  ...
  @Override protected void onCloseMessage (CloseMessage cm,
    ...
    else super.onCloseMessage( cm, channel );
  }

  @Override protected void onError (WebSocketChannel channel,
                                    Throwable error) {
    if (this.onError != null)
      this.onError.invoke( RT.map( k_callback, RT.keyword( null, "on-error" ),
                                   k_channel, channel,
                                   RT.keyword( null, "error" ), error ) );
    else super.onError( channel, error );
  }
